...
for t=1:opts.batchSize:numel(subset)
  ...
  for s=1:opts.numSubBatches
    ...
    if opts.augmentation && strcmp(mode,'train')
       % % data augmentation
        fprintf('Augmentation ');
        inputs{2} = padarray(inputs{2},[4 4 0 0]);  %pad 4 zeros
        i = randi([1 9],1,2);
        inputs{2} = inputs{2}(i(1):i(1)+32-1,i(2):i(2)+32-1,:,:); % random cropping to 32x32
        if rand > 0.5, inputs{2}=fliplr(inputs{2}) ; end   %<- random horizontally fliping
    end
