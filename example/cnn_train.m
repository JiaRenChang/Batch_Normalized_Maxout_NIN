...
for t=1:opts.batchSize:numel(subset)
  ...
  for s=1:opts.numSubBatches
    ...
    if opts.augmentation && training
       % % data augmentation
        fprintf('Augmentation ');
        im = padarray(im,[2 2 0 0]);  %pad 4 zeros
        i = randi([1 5],1,2);
        im = im(i(1):i(1)+32-1,i(2):i(2)+32-1,:,:); % random cropping to 32x32
        if rand > 0.5, im=fliplr(im) ; end   %<- random horizontally fliping
    end
