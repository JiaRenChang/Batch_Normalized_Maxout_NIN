...
%add
 lib_src{end+1} = fullfile(root,'matlab','src','bits',['nnmaxout.' ext]) ;
 mex_src{end+1} = fullfile(root,'matlab','src',['vl_maxout.' ext]) ;
% CPU-specific files
...
if opts.enableGpu
  ...
  lib_src{end+1} = fullfile(root,'matlab','src','bits','impl','maxout_gpu.cu') ;
end
